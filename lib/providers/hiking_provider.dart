...
///
/// 설계 원칙:
/// - Provider는 타이머(시간 계산)와 데이터 상태만 소유
/// - GPS 추적은 HikingScreen이 소유 (Kakao 지도 표시용)
/// - 화면은 onPositionUpdate()를 직접 호출해 Provider 데이터를 갱신
/// - 화면이 없어도 타이머는 계속 동작 → 백그라운드에서 시간이 흐름
class HikingProvider extends ChangeNotifier with WidgetsBindingObserver {
  ...
  /// HikingScreen이 등산을 시작할 때 호출. 타이머와 데이터 초기화.
  void initializeHike({
  ...
  void _onTimerTick(Timer timer) {
    if (!_isPaused) {
      _elapsedSeconds++;
      if (_isDescending) _descentSeconds++;

      if (_pedometerService != null) {
  ...
  // ─── GPS 위치 업데이트 (화면에서 직접 호출) ───

  /// HikingScreen의 GPS 콜백에서 호출. 데이터 계산 및 상태 갱신.
  ...
  Future<void> saveHikingState() async {
    ...
    try {
      ...
      await prefs.setDouble('hiking_min_alt',
          _minAltitude == double.infinity ? 0 : _minAltitude);
      ...
      await prefs.setInt('hiking_descent_start_steps', _descentStartSteps);
      ...
      await prefs.setInt(
          'hiking_saved_at', DateTime.now().millisecondsSinceEpoch);
      final positionData = _trackPositions
          .map((p) =>
  ...
  Future<bool> restoreHikingState({
    ...
    try {
      ...
      _elapsedSeconds =
          (prefs.getInt('hiking_seconds') ?? 0) + additionalSeconds;
      ...
      _descentSeconds = (prefs.getInt('hiking_descent_secs') ?? 0) +
          (_isDescending ? additionalSeconds : 0);
      _descentSteps = prefs.getInt('hiking_descent_steps') ?? 0;
      _descentStartSteps = prefs.getInt('hiking_descent_start_steps') ?? 0;
      _ascentDistance = prefs.getDouble('hiking_ascent_dist') ?? 0;
      _ascentSeconds = prefs.getInt('hiking_ascent_secs') ?? 0;
      _ascentSteps = prefs.getInt('hiking_ascent_secs') ?? 0;
      ...
      for (final data in positionData) {
        ...
        if (parts.length >= 3) {
          ...
          if (lat == null || lng == null || alt == null) continue;
          _trackPositions.add(Position(
            latitude: lat,
            longitude: lng,
            altitude: alt,
            timestamp: parts.length >= 4
  ...
  void _updateHikingNotification() {
    ...
    final totalDist =
        _isDescending ? (_ascentDistance + _descentDistance) : _totalDistance;
    final distStr = totalDist >= 1000
        ? '${(totalDist / 1000).toStringAsFixed(1)}km'
        : '${totalDist.toStringAsFixed(0)}m';
    final totalSecs =
        _elapsedSeconds + (_isDescending ? _descentSeconds : 0);
    ...
    FlutterLocalNotificationsPlugin().show(
      ...
      const NotificationDetails(
        ...
        iOS: DarwinNotificationDetails(
            presentAlert: false, presentBadge: false),
      ),
      payload: 'hiking',
    );
  }
